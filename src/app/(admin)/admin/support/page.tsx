  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-slate-500">Manage support requests from all partners.</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-lg border border-dashed border-slate-300 p-8 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">There are currently no support requests to review.</p>
        </div>
      ) : (
        <div className="overflow-x-auto rounded-lg border border-slate-200 bg-white shadow-sm">
          <table className="min-w-full divide-y divide-slate-200">
            ...
            <tbody className="divide-y divide-slate-200 bg-white">
              {requests.map((req) => (
                <tr key={req.id} className="hover:bg-slate-50 cursor-pointer" onClick={() => window.location.href = `/admin/support/${req.id}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-900">
                    ...
                    <div className="text-xs text-slate-500">{req.partner.user.email}</div>
                  </td>
