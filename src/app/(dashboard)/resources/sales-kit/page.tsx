...
export default function SalesKitPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div>
        <h1 className="text-3xl font-bold tracking-tight text-slate-900 dark:text-slate-100">Sales Kit</h1>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {assets.map((asset) => (
          <div key={asset.title} className="relative flex flex-col items-center p-6 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm text-center">
            <div className="p-3 bg-slate-50 dark:bg-slate-900/50 text-slate-700 dark:text-slate-300 rounded-full mb-4">
              <asset.icon className="h-6 w-6" />
            </div>
            <h3 className="text-lg font-medium text-slate-900 dark:text-slate-100">{asset.title}</h3>
            <p className="mt-2 text-sm text-slate-500 dark:text-slate-400 mb-6">{asset.description}</p>
